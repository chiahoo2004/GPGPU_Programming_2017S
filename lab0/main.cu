...
__global__ void Draw(char *frame) {
	...
	if (y < H and x < W) {
		...
		if (x == W-1) {
			...
			c = ':';
		} else if ( (y == 5 and x >= 18 and x <= 21 ) or
					(y == 6 and x >= 16 and x <= 21 ) or
					(y == 7 and x >= 14 and x <= 21 ) or
					(y == 8 and x >= 12 and x <= 21 ) or
					(y == 9 and x >= 10 and x <= 21 ) or
					(y == 10 and ((x >= 8 and x <= 21) or (x==33)) ) ) {
			c = '#';
		} else if (x == 33 and y >= 5 and y <= 9) {
			c = '|';
